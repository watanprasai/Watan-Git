...
import { MapBedInterface } from "../models/UI";
import { LevelInterface } from "../models/UI";
import { UserInterface } from "../models/UI";
...
function Users() {
    ...
    const submit = () => {
        let data={
	        ...
	        Comment: comm,		
	        Medicine: med,
	        CheckID: convertType(UserID),
	        LevelID: convertType(LevelID),
	        MapbID: convertType(MapBedID),
        }
        fetch("http://localhost:8080/symptom", {
        method: "POST",
        headers: {
    ...
    const [mapbeds, setMapbed] = React.useState<MapBedInterface[]>([]);
    const getMapbeds = async () => {
        const apiUrl = "http://localhost:8080/mapbeds";
   ...
   const getLevels = async () => {
      const apiUrl = "http://localhost:8080/levels";
      const requestOptions = {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      };
                ...
                >
                  <option aria-label="None" value="">
                    เตียงผู้ป่วย
                      ...
                      {mapbeds.map((item: MapBedInterface) => (
